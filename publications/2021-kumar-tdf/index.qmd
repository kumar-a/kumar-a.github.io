...
# st_read("E:/spatial-data/boundaries/tnc_terr_ecoregions/tnc_terr_ecoregions.shp", 
#         quiet = TRUE) |>
#   filter(WWF_MHTNUM < 4) |>
#   summarise() |>
#   st_write("data/tnc_tropical_forests.gpkg")

tropical_forests <- st_read("data/tnc_tropical_forests.gpkg", quiet = TRUE)
...
```{r dev='svglite'}
#| label: fig-npp
#| fig-cap: Total Net Primary Productivity (NPP) across Terrestrial Ecosystems Tropical Ecosystems are responsible for about 60% of the Total Terrestrial NPP. (1 Pg &equals; 10^15^ g). Data from @Saugier2001.
...
dat <- read.csv("data/leaf_carbon_nutrients.csv")

dat1 <- dat %>% filter(!is.na(P_green_leaf), !is.na(P_senesced_leaf), 
                       grepl("tropical|forest|Forest", Vegetation)) %>%
      dplyr::select(Vegetation, Growth_habit, P_green_leaf, P_senesced_leaf) %>%
      dplyr::group_by(Vegetation)

dat1$biome <- dat1$Vegetation

dat1$biome[grepl("[Tt]ropical", dat1$biome)] <- "Tropical"

dat1$biome[grepl("[Tt]ropical", dat1$biome)] <- "Tropical"
dat1$biome[grepl("[Ss]ubtropical", dat1$biome)] <- "Subtropical"
dat1$biome[grepl("[Tt]emperate", dat1$biome)] <- "Temperate"
dat1$biome[grepl("[Pp]ine|[Cc]oniferous", dat1$biome)] <- "Coniferous"

dat1$biome[grepl("Mixed mesophytic forest", dat1$biome)] <- "Temperate"

dat1 <- dat1 %>% filter(grepl("Tropical|Temperate", biome))  %>%
      dplyr::mutate(nur = (1 - (P_senesced_leaf/P_green_leaf))*100)

ggplot(data = dat1, aes(x = P_green_leaf, y = P_senesced_leaf)) + 
      geom_point(aes(fill = biome), size = 4, shape = 21, alpha = 0.5) + 
      geom_smooth(method = "lm", alpha = 0.2, size = 1.3) + 
      
      ggpubr::stat_cor(method = "pearson", label.x = 0.05, label.y = 0.1875, 
               r.accuracy = 0.01, p.accuracy = 0.001, size = 5) + 
      
      labs(x = "Green Leaf P (% dry mass)", 
           y = "Senesced Leaf P (% dry mass)") +
      theme_bw() +
      theme(legend.position = c(0.16, 0.9), legend.title = element_blank(),
            legend.text = element_text(size = 14), legend.key.height = unit(1.1, "cm"),
